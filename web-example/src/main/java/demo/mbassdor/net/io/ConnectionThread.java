...
import demo.mbassdor.core.event.connection.IrcDisconnect;
import demo.mbassdor.core.event.message.recieve.ReceiveChannelMode;
import demo.mbassdor.core.event.message.recieve.ReceiveInvite;
import demo.mbassdor.core.event.message.recieve.ReceiveJoin;
import demo.mbassdor.core.event.message.recieve.ReceiveKick;
import demo.mbassdor.core.event.message.recieve.ReceiveMode;
import demo.mbassdor.core.event.message.recieve.ReceiveNick;
import demo.mbassdor.core.event.message.recieve.ReceiveNotice;
...
import demo.mbassdor.core.event.message.recieve.ReceivePrivmsg;
import demo.mbassdor.core.event.message.recieve.ReceiveQuit;
import demo.mbassdor.core.event.message.recieve.ReceiveReply;
import demo.mbassdor.core.event.message.recieve.ReceiveTopic;
import demo.mbassdor.core.event.message.recieve.ReceiveUserMode;
import demo.mbassdor.net.mbassdor.MBassadorWrapper;
import demo.mbassdor.net.message.InvalidMessageCommandException;
import demo.mbassdor.net.message.InvalidMessageException;
import demo.mbassdor.net.message.InvalidMessageParamException;
import demo.mbassdor.net.message.InvalidMessagePrefixException;
...
public class ConnectionThread extends Thread {
	...
	/**
	 * How long a read on the socket will block before throwing
	...
	private void createOutput(Socket socket) throws IOException {
		...
		synchronized (outputWriterMutex) {
			...
			// Don't want to carry forward the old stuff from before
			// reconnecting
			outputQueue.clear();
		}
	...
	public void run() {
			...
			public void run() {
				...
				while (!Thread.interrupted()) {
					...
					if (outputWriter != null) {
						...
						//Logger.error("First connection has not been established, can't send IRC message '" + rawCommand
						//		+ "'");
					}

					try {
						// Sending messages too close to each other can cause
						// problems on some servers, if the order of
						// the messages matters (ex: NICK must proceed USER at
		...
		while (state != RunState.END) {
			...
			//	try {
					...
					socket =null; //socketFactory.createSocket(endpoint.getAddress(), endpoint.getPort());
...
//					input = new BufferedReader(new InputStreamReader(inputStream, IoConfig.CHARSET));
					...
					state = RunState.READ;
//				} catch (IOException e) {
//					logger.error( "Failed to connect socket, sleeping before retrying.",e);
				//}
...
//										+ messageEvent);
							...
							if (messageEvent != null) {
//								Logger.trace("Publishing message event type: " + messageEvent.getClass().getSimpleName()
//										+ ", content: " + messageEvent);
					...
					// Sending some data is pretty much the only way to tell if
					// the TCP connection is still alive. We
					// don't really care about the reply, just that sending it
					// doesn't cause a connection error.
					try {
